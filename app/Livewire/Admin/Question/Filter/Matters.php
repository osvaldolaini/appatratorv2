...
class Matters extends Component
{
    ...
    public $model_id;
    public $registerId;
    public $tabs = 'matérias';
    public $disciplines;

    //Dados da tabela
    public $model = "App\Models\Admin\Questions\Filters\Matter"; //Model principal
    public $modelId="id"; //Ex: 'table.id' or 'id'
    public $search;
    public $relationTables; //Relacionamentos ( table , key , foreingKey )
    public $customSearch; //Colunas personalizadas, customizar no model
    public $columnsInclude = 'title,nick,discipline_id,active';
    ...
    {
        $this->disciplines = Discipline::where('active', 1)->get();
        return view('livewire.admin.questions.filters.matter', [
    ...
    //UPDATE
    public function showModalUpdate(Matter $matter)
    {
        $this->resetAll();

        $this->model_id         = $matter->id;
        $this->title            = $matter->title;
        $this->nick             = $matter->nick;
        $this->discipline_id    = $matter->discipline_id;

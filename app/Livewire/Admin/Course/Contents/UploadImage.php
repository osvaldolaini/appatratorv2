...
class UploadImage extends Component
{
    ...
    public function mount($id)
    {
        if ($id) {
            ...
            if (Storage::exists('public/modules/' . $this->module.'/content/'.$this->id.'/thumb')) {
                $this->photo = true;
            } else {
                $this->photo = false;
            }
    ...
    {
        return view('livewire.admin.courses.contents.upload-image');
    }
    ...
    public function updated($property)
    {
        if ($property === 'uploadimage') {
            ...
            if (isset($this->uploadimage)) {
                ...
                $new_name = $this->content->code . '.jpg';
    ...
    {
        Storage::deleteDirectory('public/modules/' . $this->module.'/content/'.$this->id.'/thumb');
    ...
    {
        Storage::makeDirectory('public/modules/' . $this->module.'/content/'.$this->id.'/thumb');

        // dd('storage/public/livewire-tmp/' . $image);
        $img = explode('.', $image);
        $logoWebp = Image::make('storage/livewire-tmp/' . $image);
        $logoWebp->encode('webp', 80);
        $logoWebp->save('storage/modules/' . $this->module.'/content/'.$this->id.'/thumb/' . $img[0] . '.webp');

        $logoPng = Image::make('storage/livewire-tmp/' . $image);
        $logoPng->encode('png', 80);
        $logoPng->save('storage/modules/' . $this->module.'/content/'.$this->id.'/thumb/' . $img[0] . '.jpg');
    }

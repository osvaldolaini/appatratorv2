...
class Topics extends Component
{
    use WithPagination;
    public Topic $topic;
    public $breadcrumb = 'Temas de redação';
    ...
    //Dados da tabela
    public $model = "App\Models\Admin\Essay\Topic"; //Model principal
    public $modelId="topics.id"; //Ex: 'table.id' or 'id'
    public $search;
    public $relationTables = 'essays,essays.id,topics.essay_id'; //Relacionamentos ( table , key , foreingKey )
    public $customSearch; //Colunas personalizadas, customizar no model
    public $columnsInclude = 'topics.title,topics.active,topics.essay_id';
    public $searchable = 'topics.title'; //Colunas pesquisadas no banco de dados
    public $sort = "topics.title,asc"; //Ordenação da tabela se for mais de uma dividir com "|"
    ...
    {
        return view('livewire.admin.essay.topic', [
    ...
    //UPDATE
    public function showModalUpdate(Topic $topic)
    {
        $this->resetAll();

        $this->model_id         = $topic->id;
        $this->title            = $topic->title;
        $this->essay_id            = $topic->essay_id;
    ...
    private function getData()
    {
        if (Auth::user()->group == 'super-admin') {
            ...
            $query = $this->model::query();
            $query = $query->where('topics.active', '<=', 1);
        }

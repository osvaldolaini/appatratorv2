...
class ContestDisciplines extends Component
{
    use WithPagination;
    ...
    //Dados da tabela
    public $model = "App\Models\Admin\Mentoring\ContestDiscipline"; //Model principal
    public $modelId="contest_disciplines.id"; //Ex: 'table.id' or 'id'
    public $search;
    public $relationTables = 'contests,contests.id,contest_disciplines.contest_id'; //Relacionamentos ( table , key , foreingKey )
    public $customSearch; //Colunas personalizadas, customizar no model
    public $columnsInclude = 'contests.title,contest_disciplines.id,contest_disciplines.title,contest_disciplines.active,contest_disciplines.nick,contest_disciplines.order';
    public $searchable = 'contests.title,contest_disciplines.title'; //Colunas pesquisadas no banco de dados
    ...
    {
        $this->contest = Contest::find($contest);
        $this->contest_id = $contest;
        $this->breadcrumb.= $this->contest->title;
    }

    public function render()
    {
        return view('livewire.admin.mentoring.contest-disciplines', [
    ...
    {
        $this->rules = [
                'title'=>'required|min:4|max:255|unique:contest_disciplines',
        ];
        $this->validate();

        ContestDiscipline::create([
            'title'     =>$this->title,
            'active'    =>$this->active,
            'nick'      =>$this->nick,
            'order'       =>$this->order,
    ...
    public function showModalRead($id)
    {
        ...
        if (isset($id)) {
            $data = ContestDiscipline::where('id',$id)->first();
            // dd($data);
            $this->detail = [
    ...
    //UPDATE
    public function showModalUpdate($contestDiscipline)
    {
        $contestDiscipline = ContestDiscipline::find($contestDiscipline);
        $this->model_id = $contestDiscipline->id;
        $this->title    = $contestDiscipline->title;
        $this->nick     = $contestDiscipline->nick;
        $this->order    = $contestDiscipline->order;
    ...
    public function update()
    {
        ...
        ContestDiscipline::updateOrCreate([
            'id'=>$this->model_id,
        ],[
            'title'     =>$this->title,
            'nick'      =>$this->nick,
    ...
    private function getData()
    {
        if (Auth::user()->group == 'super-admin') {
            ...
            $query = $this->model::query();
            $query = $query->where('contest_disciplines.active', '<=', 1);
        }
        $query = $query->where('contest_disciplines.contest_id', $this->contest_id);

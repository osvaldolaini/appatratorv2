        ...
        <form>
            <fieldset class="grid grid-cols-12 gap-2 pb-6 rounded-md dark:bg-gray-900 items-start">
                <div class="col-span-full sm:col-span-12">
                    <label for="title">*Selecione as categorias</label>
                    <select wire:change="insert()" wire:model="category_id"
                ...
                <div class="col-span-full">
                    <h2 class="text-bold">Categorias selecionadas (Clique no X para excluir)</h2>
                </div>

                @if ($categoryCourse)
                    <div class="col-span-full py-5 rounded-md border-2  border-gray-500 border-dashed bg-gray-200">
                        <div class="flex flex-row ml-2">
                            @foreach ($categoryCourse as $category)
                                <div class="badge badge-outline badge-error gap-1 mr-2">
                                    {{ $category->category->title }}
                                    <svg wire:click="delete({{ $category->id }})" xmlns="http://www.w3.org/2000/svg"

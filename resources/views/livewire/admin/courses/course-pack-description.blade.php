        ...
        <form wire:submit="save">
            <fieldset class="grid grid-cols-12 gap-2 py-6 rounded-md dark:bg-gray-900">
                {{-- <div class="col-span-full " wire:ignore>
                        <label for="large_description">Descrição do curso</label>
                        <textarea id="large_description" class="w-full " wire:model="large_description">
                                {{ old('large_description', $large_description ?? '') }}
                            </textarea>
                    </div> --}}
                <div class="col-span-full" wire:ignore>
                    <textarea wire:model.defer="large_description" id="editor">{{ old('large_description', $large_description ?? '') }}</textarea>
            ...
            $('#large_description').summernote({
                ...
                callbacks: {
                    onChange: function(contents, $editable) {
                        @this.set('large_description', contents);
                    }
                }
            });
        </script>
        ...
        ClassicEditor
            ...
            .then(editor => {
                editor.model.document.on('change:data', () => {
                    @this.set('large_description', editor.getData())
                });
            })
            .catch(error => {
                console.error(error);

<div class="w-100">
    <button wire:click="showQuestions({{ $matter_id }})" class="btn btn-outline btn-accent btn-sm">Questões
    </button>
    {{-- MODAL INVOLVED --}}
            ...
            <form action="#" wire:submit.prevent="store()" wire.loading.attr='disable'>
                <div class="grid gap-4 mb-1 sm:grid-cols-3 sm:gap-6 sm:mb-5">
                    ...
                    <div class="col-span-3 sm:col-span-1">
                        ...
                        <input type="text" wire:model="hits" name="hits" id="hits"
                            placeholder="Acertos " required=""
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                        @error('hits')
                            <span class="error">{{ $message }}</span>
                        @enderror

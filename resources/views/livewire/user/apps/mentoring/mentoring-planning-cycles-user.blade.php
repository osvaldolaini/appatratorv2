            ...
            <div
                ...
                <div class="group flex ">
                    ...
                    hover:bg-blue-600 dark:hover:bg-blue-500 dark:bg-blue-600">
                        ...
                        <span>Inserir matéria no ciclo</span>
                    </button>
                </div>
            </div>
        </div>
        ...
        <div class=" bg-white dark:bg-gray-800 my-6 px-4">
            <div class="-mx-4  overflow-x-auto sm:-mx-6 lg:-mx-8">
                @if ($planningCycleUser->count() > 0)
                    <div class="grid grid-cols-6 gap-4 mx-5 px-2 pb-48" wire:model="{{ $planningCycleUser }}">
    ...
    <x-dialog-modal wire:model="showModalCreate" class="mt-0">
        ...
        <x-slot name="content">
            <form action="#" wire:submit.prevent="store()" wire.loading.attr='disable'>
                <div class="grid gap-2 mb-1 sm:grid-cols-5 sm:gap-6 sm:mb-5">
                    <div class="col-span-5 sm:col-span-3">
                        <label for="contest_discipline_id" class="block text-sm font-medium text-gray-900 dark:text-white">
                            Matéria
                        </label>
                        <select wire:model="contest_discipline_id" required=""
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            <option value="">Selecione</option>
                            @foreach ($disciplines as $discipline)
                            {{-- @if (!$cycle->contains($discipline->id)) --}}
                                <option value="{{ $discipline->id }}">{{ strtoupper($discipline->title) }}</option>
                            {{-- @endif --}}
                            @endforeach
                        </select>
                ...
                <div class="grid gap-2 mb-1 sm:grid-cols-5 sm:gap-6 sm:mb-5">
                    <div class="col-span-5 sm:col-span-3">
                        ...
                        <select wire:model="contest_discipline_id" required=""
                            ...
                            <option value="">Selecione</option>
                            @foreach ($disciplines as $discipline)
                                {{-- @if (!$cycle->contains($discipline->id) or $discipline->id == $contest_discipline_id) --}}
                                    <option value="{{ $discipline->id }}">{{ strtoupper($discipline->title) }}</option>
                                {{-- @endif --}}
                            @endforeach
                        </select>
